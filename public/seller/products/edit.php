...
$product = $product_obj->getById($product_id);

if (!$product || $product['seller_id'] != $seller_info['seller_id']) {
    setFlashMessage('Unauthorized: You cannot edit this product.', 'error');
    secureRedirect(BASE_URL . '/public/seller/products/list.php');
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!verifyCSRFToken($_POST['csrf_token'] ?? '')) {
        ...
        if (empty($product_name) || empty($product_description) || $srp <= 0 || $quantity < 0) {
            ...
            if (empty($error)) {
                $result = $product_obj->update(
                    $product_id,
                    $product_name,
                    $product_description,
                    $srp,
                    $quantity,
                    $image_path !== $product['image_path'] ? $image_path : null
                );

                if ($result['success']) {
                    $success = 'Product updated successfully!';
                    logSecurityEvent('product_updated', 'Product updated: ' . $product_name, $_SESSION['user_id']);
...
<body class="bg-gray-50">
    ...
    <div class="max-w-2xl mx-auto px-4 py-12">
        ...
        <p class="text-gray-600 mb-8"><?php echo htmlspecialchars($product['product_name']); ?></p>
            ...
            <form method="POST" enctype="multipart/form-data">
                ...
                <div class="mb-6">
                    ...
                    <textarea 
                        ...
                        rows="5"
                        class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none transition"
                    ><?php echo htmlspecialchars($product['product_description']); ?></textarea>
                </div>
                ...
                <div class="mb-6 grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input 
                            ...
                            step="0.01"
                            min="0.01"
                            value="<?php echo number_format($product['srp'], 2, '.', ''); ?>"
                            class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none transition"
                        >
                    </div>
                ...
                <!-- Product Image -->
                <div class="mb-6">
                    <label for="product_image" class="block text-sm font-medium text-gray-700 mb-2">Product Image (Optional)</label>
                    
                    <!-- Current Image -->
                    <?php if (!empty($product['image_path'])): ?>
                        <div class="mb-4">
                            <p class="text-sm text-gray-600 mb-2">Current image:</p>
                            <img src="<?php echo getProductImageUrl($product['image_path']); ?>" alt="Product" class="max-h-40 rounded-lg border border-gray-300">
                        </div>
                    <?php endif; ?>
