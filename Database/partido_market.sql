-- Partido Product Online Market Hub Database Schema
-- Stage 2: Seller Dashboard & Product Management

CREATE DATABASE IF NOT EXISTS partido_market;
USE partido_market;
...
-- Deals Table - Local marketplace deals (Stage 2)
CREATE TABLE deals (
...
-- Conversations Table - Messenger conversations linked to deals (Stage 4)
...
-- Messages Table - Chat messages for conversations (Stage 4)
...
-- Ratings Table - Buyer ratings for sellers after deal completion (Stage 5)
...
-- Notifications Table - Event notifications for users (Stage 5)
...
-- Admin Actions Log Table (Stage 6)
...
-- Flag Reports Table (Stage 6) - For user reports on content
...
-- Add flagged column to ratings table (Stage 6)
ALTER TABLE ratings ADD COLUMN flagged BOOLEAN DEFAULT FALSE;

-- Insert sample admin user (email: admin@partido.com, password: Admin@123)
